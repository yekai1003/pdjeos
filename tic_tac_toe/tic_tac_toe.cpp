...
struct impl {
   ...
   void on(const create& c) {
        print("con create ",name{c.host},",",name{c.challenger});
      require_auth(c.host);
      eosio_assert(c.challenger != c.host, "challenger shouldn't be the same as host");

      // Check if game already exists
      games existing_host_games(c.host, c.host);
   ...
   void on(const restart& r) {
      ...
      games existing_host_games(r.host, r.host);
   ...
   void on(const move& m) {
      ...
      // Check if game exists
      games existing_host_games(m.host, m.host);
   ...
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
           print("call apply:",code,",",code_account);
      if (code == code_account) {

         if (action == N(create)) {
                //impl::on(unpack_action_data<create>());
                impl::on(eosio::unpack_action_data<create>());
         } else if (action == N(restart)) {
                impl::on(eosio::unpack_action_data<restart>());
         } else if (action == N(close)) {
                impl::on(eosio::unpack_action_data<close>());
         } else if (action == N(move)) {
                impl::on(eosio::unpack_action_data<move>());
         }
      }
